...
const deleteRequest = (event) => {
    ...
    form.delete(route('deleteRequest',id))
    ...
    <template>
        <Head title="My Property Requests" />
        <DashboardLayout>
            <div class="dashborad-box">
                <h4 class="title">Property Requests!</h4>
                <Link :href="route('createRequest')">
                    <button class="inline-flex items-center px-4 py-2 bg-gray-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest hover:bg-gray-700 active:bg-gray-900 focus:outline-none focus:border-gray-900 focus:shadow-outline-gray transition ease-in-out duration-150">Create Request!</button>
                </Link>
                <div class="row">
                    ...
                    <div v-for="prop in props.data" :key="prop.id" class="col-lg-3 col-md-3 shadow rounded mt-4 offset-md-1 offset-lg-1">
                        <table class="table ">
                            ...
                            <tr>
                                ...
                                <td data-label="Column 2">₦{{ prop.price.toLocaleString() }}</td>
                            </tr>
                        ...
                        <div class="d-flex flex-row justify-content-center p-4 f-20">
                            <Link :href="route('editRequest',prop.id)"><i class="fas fa-pencil-alt"></i></Link>
                            <a :id="prop.id" :onclick="deleteRequest"  class="ml-5"><i :id="prop.id" class="fas fa-trash"></i></a>
                            <Link :href="route('requestDetails',prop.id)" class="ml-5"><i class="fas fa-eye"></i></Link>
                        </div>
